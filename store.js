...
export function createStore(stageWidth, stageHeight) {
    ...
    switch (event.type) {
      ...
      case 'MOVE_DOWN':
        return state.activeShape == null
          ? spawn(state)
          : pipe(state, moveDown, hasCollision) //tomar el estado de cada colision
          ? isTopOut(state) // 1
            ? endGame(state) // 2
            : spawn(state) // 3
      ...
      default:
        return state; // retorno este coso
    }
  };
  ...
  return {
    ...
    dispatch: event => {
      ...
      if (newState !== state) {
        state = newState; // nuevo estaado
        subscribers.forEach(subscriber => { //suscribo a esperar el estado
          subscriber(state);
        });
      }
    },
...
function clearFullRows(state) {
  ...
  const clearedMatrix = matrix.filter(row => !row.every(Boolean)).reverse(); //reversa si
    ...
    matrix: createMatrix(stageWidth, stageHeight)
      .map((row, y) => {
        return row.map((cell, x) => {
          return clearedMatrix[y]?.[x]; // tiene q limpiar la matrizzzzzzzzz
        });
      })
      .reverse(),
  };
