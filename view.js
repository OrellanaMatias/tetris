export function createRenderer({ ctx, bufferWidth, bufferHeight, unit }) {
  ...
  return state => {
    ...
    state.activeShape?.matrix.forEach((row, y) => {
      row.forEach((cell, x) => {
        if (cell) {
          ...
          renderCell(xGlobal, yGlobal, state.activeShape.fill); // aaaaaaaaaaaaaaaaa
        }
      });
    });
    ...
    ctx.textAlign = 'right';
    ctx.fillStyle = 'black';
    ctx.fillText(state.score, bufferWidth - 12, 32); // este alto esta mal
    ...
    if (state.isGameOver) {
      ...
      ctx.fillStyle = 'black';
      ctx.fillText('SS RE BOT!', bufferWidth / 2, bufferHeight / 2);
